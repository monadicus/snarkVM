...
impl FromBytes for PartialSolution {
    /// Reads the partial solution from the buffer.
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        let epoch_hash = BlockHash::read_le(&mut reader)?;
...
impl ToBytes for PartialSolution {
...
mod tests {
    ...
    fn test_bytes() -> Result<()> {
        ...
        assert!(PartialSolution::read_le(&expected_bytes[1..]).is_err());
