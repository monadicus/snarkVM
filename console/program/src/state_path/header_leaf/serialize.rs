...
impl Serialize for HeaderLeaf {
    /// Serializes the leaf into string or bytes.
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        match serializer.is_human_readable() {
            true => {
                let mut leaf = serializer.serialize_struct("HeaderLeaf", 2)?;
...
impl<'de> Deserialize<'de> for HeaderLeaf {
...
mod tests {
    ...
    fn test_serde_json() -> Result<()> {
        ...
        // Deserialize
        assert_eq!(expected, HeaderLeaf::from_str(expected_string)?);
        assert_eq!(expected, serde_json::from_str(&candidate_string)?);
