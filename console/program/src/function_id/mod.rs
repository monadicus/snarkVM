...
pub fn compute_function_id(network_id: &U16, program_id: &ProgramID, function_name: &Identifier) -> Result<Field> {
    AleoNetwork::hash_bhp1024(
        &(
            *network_id,
            U8::new(program_id.name().size_in_bits()),
            program_id.name(),
            U8::new(program_id.network().size_in_bits()),
            program_id.network(),
            U8::new(function_name.size_in_bits()),
            function_name,
        )
            .to_bits_le(),
    )
