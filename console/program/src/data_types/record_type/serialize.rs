...
impl<'de> Deserialize<'de> for RecordType {
    ...
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        match deserializer.is_human_readable() {
            ...
            false => FromBytesDeserializer::<Self>::deserialize_with_size_encoding(deserializer, "record type"),
        }
    }
}
...
mod tests {
    ...
    fn test_serde_json() {
        for case in TEST_CASES.iter() {
            check_serde_json(RecordType::from_str(case).unwrap());
        }
    }
    ...
    fn test_bincode() {
        for case in TEST_CASES.iter() {
            check_bincode(RecordType::from_str(case).unwrap());
        }
    }
}
