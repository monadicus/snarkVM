...
impl Parser for ArrayType {
    ...
    fn parse(string: &str) -> ParserResult<Self> {
        ...
        fn parse_inner_element_type(string: &str) -> ParserResult<PlaintextType> {
        ...
        let (string, dimensions) = map_res(many0_count(pair(tag("["), Sanitizer::parse_whitespaces)), |dimensions| {
            ...
            } else if dimensions > AleoNetwork::MAX_DATA_DEPTH {
                Err(format!("Array type exceeds the maximum depth of {}.", AleoNetwork::MAX_DATA_DEPTH))
            } else {
                Ok(dimensions)
            }
        })(string)?;
