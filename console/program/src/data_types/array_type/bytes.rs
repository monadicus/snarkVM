...
impl FromBytes for ArrayType {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        ...
        match dimensions {
            ...
            dimensions if dimensions <= AleoNetwork::MAX_DATA_DEPTH => (),
            _ => {
                return Err(error(format!("Array type exceeds the maximum depth of {}.", AleoNetwork::MAX_DATA_DEPTH)));
            }
        }
...
impl ToBytes for ArrayType {
    fn write_le<W: Write>(&self, mut writer: W) -> IoResult<()> {
        ...
        for _ in 1..AleoNetwork::MAX_DATA_DEPTH {
