...
pub struct Pedersen<const NUM_BITS: u8> {
    ...
    base_window: Arc<Vec<Group>>,
    /// The random base window for the Pedersen commitment.
    random_base_window: Arc<Vec<Group>>,
}

impl<const NUM_BITS: u8> Pedersen<NUM_BITS> {
    /// Initializes a new instance of Pedersen with the given setup message.
    pub fn setup(message: &str) -> Self {
        // Construct an indexed message to attempt to sample a base.
        let (generator, _, _) = Blake2Xs::hash_to_curve::<ConsoleAffine>(&format!("Aleo.Pedersen.Base.{message}"));
        // Construct the window with the base.
        let mut base_window = vec![Group::zero(); NUM_BITS as usize];
        {
        ...
        let (generator, _, _) =
            Blake2Xs::hash_to_curve::<ConsoleAffine>(&format!("Aleo.Pedersen.RandomBase.{message}"));
        ...
        let mut random_base = Vec::with_capacity(Scalar::size_in_bits());
        {
            ...
            assert_eq!(random_base.len(), Scalar::size_in_bits());
        }
    ...
    pub fn base_window(&self) -> &Arc<Vec<Group>> {
        &self.base_window
    }

    /// Returns the random base window.
    pub fn random_base_window(&self) -> &Arc<Vec<Group>> {
