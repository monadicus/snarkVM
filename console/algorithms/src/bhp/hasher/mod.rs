...
pub struct BHPHasher<const NUM_WINDOWS: u8, const WINDOW_SIZE: u8> {
    ...
    bases: Arc<Vec<Vec<Group>>>,
    /// The bases lookup table for the BHP hash.
    bases_lookup: Arc<Vec<Vec<[Group; BHP_LOOKUP_SIZE]>>>,
    /// The random base for the BHP commitment.
    random_base: Arc<Vec<Group>>,
}
...
impl<const NUM_WINDOWS: u8, const WINDOW_SIZE: u8> BHPHasher<NUM_WINDOWS, WINDOW_SIZE> {
    ...
    pub fn setup(domain: &str) -> Result<Self> {
        ...
        let mut range = ConsoleBigInteger::from(2_u64);
        while range < ConsoleScalar::modulus_minus_one_div_two() {
            // range < (p-1)/2
        ...
        let bases = (0..NUM_WINDOWS)
            .map(|index| {
                ...
                let (generator, _, _) = Blake2Xs::hash_to_curve::<ConsoleAffine>(&format!(
                    "Aleo.BHP.{NUM_WINDOWS}.{WINDOW_SIZE}.{domain}.{index}"
                ));
                let mut base = Group::new(generator);
        ...
        let bases_lookup = bases
            ...
            .map(|x| {
                x.iter()
                    .map(|g| {
                        let mut lookup = [Group::zero(); BHP_LOOKUP_SIZE];
            ...
            .collect::<Vec<Vec<[Group; BHP_LOOKUP_SIZE]>>>();
        ...
        let (generator, _, _) = Blake2Xs::hash_to_curve::<ConsoleAffine>(&format!(
            "Aleo.BHP.{NUM_WINDOWS}.{WINDOW_SIZE}.{domain}.Randomizer"
        ));
        let mut base_power = Group::new(generator);
        let mut random_base = Vec::with_capacity(Scalar::size_in_bits());
        for _ in 0..Scalar::size_in_bits() {
            random_base.push(base_power);
            base_power = base_power.double();
        }
        ensure!(
            random_base.len() == Scalar::size_in_bits(),
    ...
    /// Returns the bases.
    pub fn bases(&self) -> &Arc<Vec<Vec<Group>>> {
        &self.bases
    }

    /// Returns the random base window.
    pub fn random_base(&self) -> &Arc<Vec<Group>> {
        &self.random_base
    }
}
